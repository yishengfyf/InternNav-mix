#!/usr/bin/env bash
# Same 5 episodes: Frozen-S2 control -> strict-only S2-loop candidate active.
...
RUNNING_NAME="stage21c_strict_loop_active_paired5_running_${PIPELINE_TAG}"
SUCCESS_NAME="stage21c_strict_loop_active_paired5_return_${PIPELINE_TAG}"
FAILURE_NAME="stage21c_strict_loop_active_paired5_failure_return_${PIPELINE_TAG}"
WORK_DIR="${RETURN_ROOT}/${RUNNING_NAME}"
SUCCESS_DEST="${RETURN_ROOT}/${SUCCESS_NAME}"
FAILURE_DEST="${RETURN_ROOT}/${FAILURE_NAME}"
CONTROL_NAME="compare_vlmap_stage21c_strict_loop_control_5ep_${PIPELINE_TAG}"
ACTIVE_NAME="compare_vlmap_stage21c_strict_loop_active_5ep_${PIPELINE_TAG}"
...
Control: Frozen S2/NextDiT, all recovery shadow-only.
Treatment: only S2-loop strict_intervention may rewrite one coarse recovery pixel per episode.
adapter_candidate/abstain: Frozen S2 hold.
Main S2/scorer/NextDiT parameters: frozen.
EOF
}
...
package_failure() {
  ...
  if [[ -d "${WORK_DIR}" ]]; then
    ...
    echo "STAGE21C_PAIRED_ACTIVE_STATUS=failed"
...
bash scripts/eval/bash/stage21_torchrun_eval.sh \
  --config scripts/eval/configs/habitat_dual_system_vlmap_stage21c_strict_loop_active_tiny_cfg.py
...
cp -a "${ACTIVE_ROOT}/stage21c_strict_loop_active_paired_audit.json" "${WORK_DIR}/"
...
echo "STAGE21C_PAIRED_ACTIVE_STATUS=complete"
