...
NPROC=${STAGE23C_NPROC_PER_NODE:-4}
RUN_NAME="compare_vlmap_stage23c_semantic_scene_audit_${PIPELINE_TAG}"
RUN_ROOT="logs/habitat/${RUN_NAME}"
WORK_DIR="${RETURN_ROOT}/stage23c_semantic_scene_audit_smoke1_running_${PIPELINE_TAG}"
SUCCESS_DEST="${RETURN_ROOT}/stage23c_semantic_scene_audit_smoke1_return_${PIPELINE_TAG}"
FAILURE_DEST="${RETURN_ROOT}/stage23c_semantic_scene_audit_smoke1_failure_return_${PIPELINE_TAG}"
...
package_failure() {
  ...
  echo "STAGE23C_SEMANTIC_STATUS=failed"
...
FAILED_STAGE=semantic_scene_shadow_evaluation
...
python3 scripts/eval/analyze_stage23c_semantic_scene.py \
  ...
  --output "${RUN_ROOT}/stage23c_semantic_scene_audit.json"
...
Stage23C compares sparse semantic anchors with Habitat semantic_scene object/
region centers after an episode. It is an independent annotation audit only:
no semantic GT is passed to OCC, S2, candidate generation, triage, or actions.
Nearest-center and category agreement are diagnostics, not voxel-level GT.
EOF
find "${WORK_DIR}" -type f | sort > "${WORK_DIR}/RETURN_MANIFEST.txt"
mv "${WORK_DIR}" "${SUCCESS_DEST}"
PIPELINE_COMPLETE=1
echo "STAGE23C_SEMANTIC_STATUS=complete"
