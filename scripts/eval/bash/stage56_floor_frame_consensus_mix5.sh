...
MANIFEST=${STAGE56_MANIFEST:?Set STAGE56_MANIFEST}
TAG=${STAGE56_PIPELINE_TAG:-$(date +%Y%m%d_%H%M%S)}
RETURN_ROOT=${STAGE56_RETURN_ROOT:-results/stage_17}
RUN_NAME="compare_vlmap_stage56_floor_frame_consensus_${TAG}"
WORK_DIR="${RETURN_ROOT}/stage56_floor_frame_consensus_running_${TAG}"
DEST="${RETURN_ROOT}/stage56_floor_frame_consensus_return_${TAG}"
FAILURE_DEST="${RETURN_ROOT}/stage56_floor_frame_consensus_failure_return_${TAG}"
CONFIG=scripts/eval/configs/habitat_dual_system_vlmap_stage56_floor_frame_consensus_cfg.py
COMPLETE=0
...
python3 -m pytest -q \
  ...
  tests/unit_test/test_stage56_floor_frame_consensus.py
...
STAGE23A_EVAL_PORT=3560 NPROC_PER_NODE=4 MASTER_PORT=3570 \
CUDA_VISIBLE_DEVICES="${STAGE56_CUDA_VISIBLE_DEVICES:-0,1,2,3}" \
bash scripts/eval/bash/stage21_torchrun_eval.sh --config "${CONFIG}"

python3 scripts/eval/analyze_stage56_floor_frame_consensus.py \
  --run-root "logs/habitat/${RUN_NAME}" \
  --manifest "${MANIFEST}" \
  --output "${WORK_DIR}/stage56_floor_frame_consensus_audit.json" \
  --require-all
