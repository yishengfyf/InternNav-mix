#!/usr/bin/env bash
# Replay paired500's 11 strict episodes and audit executed route versus ray-free OCC.
...
REFERENCE_ROOT=${STAGE22_ROUTE_REFERENCE_ROOT:-results/stage_17/stage21c_path_reobserve_active_paired500_overnight_failure_return_paired500_20260817_015559/partial_runs/control}
...
RUN_NAME="compare_vlmap_stage22a_executed_route_occ_shadow11_${PIPELINE_TAG}"
RUN_ROOT="logs/habitat/${RUN_NAME}"
RUNNING_NAME="stage22a_executed_route_occ_shadow11_running_${PIPELINE_TAG}"
SUCCESS_NAME="stage22a_executed_route_occ_shadow11_return_${PIPELINE_TAG}"
FAILURE_NAME="stage22a_executed_route_occ_shadow11_failure_return_${PIPELINE_TAG}"
...
package_failure() {
  ...
  echo "STAGE22A_EXECUTED_ROUTE_OCC_STATUS=failed"
...
STAGE22_ROUTE_EVAL_PORT=${STAGE22_ROUTE_EVAL_PORT:-2561} \
NPROC_PER_NODE=4 MASTER_PORT=${STAGE22_ROUTE_MASTER_PORT:-2562} \
bash scripts/eval/bash/stage21_torchrun_eval.sh \
  --config scripts/eval/configs/habitat_dual_system_vlmap_stage22a_executed_route_occ_shadow_cfg.py

FAILED_STAGE=route_occ_automatic_audit
python3 scripts/eval/analyze_stage22a_executed_route_occ_audit.py \
  --run-root "${RUN_ROOT}" --expected-episodes "${EXPECTED_EPISODES}" \
  --seed-manifest "${MANIFEST}" --reference-root "${REFERENCE_ROOT}" \
  --output "${RUN_ROOT}/stage22a_executed_route_occ_shadow_audit.json" \
  --require-all
...
Stage22A explicit-seed replay of the 11 episodes containing 12 strict loop events
in the Stage21c paired500 Frozen control. The run only compares the executed
pose chain with ray-derived OCC free/unknown/occupied state and known-free BFS.
No route cell is promoted to free; no S2 output, action, or recovery context is changed.
EOF
find "${WORK_DIR}" -type f | sort > "${WORK_DIR}/RETURN_MANIFEST.txt"
mv "${WORK_DIR}" "${SUCCESS_DEST}"
PIPELINE_COMPLETE=1

echo "STAGE22A_EXECUTED_ROUTE_OCC_STATUS=complete"
