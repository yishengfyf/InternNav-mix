#!/usr/bin/env bash
# Stage23A: two-episode current-pose versus GT-height SparseOcc audit.
set -euo pipefail
...
MANIFEST=${STAGE23A_MANIFEST:-scripts/eval/manifests/stage23a_pose_audit2_episode_seed_replay.json}
...
CUDA_DEVICES=${STAGE21_CUDA_VISIBLE_DEVICES:-0,1}
RUN_NAME="compare_vlmap_stage23a_pose_occ_audit2_${PIPELINE_TAG}"
RUN_ROOT="logs/habitat/${RUN_NAME}"
WORK_DIR="${RETURN_ROOT}/stage23a_pose_occ_audit2_running_${PIPELINE_TAG}"
SUCCESS_DEST="${RETURN_ROOT}/stage23a_pose_occ_audit2_return_${PIPELINE_TAG}"
FAILURE_DEST="${RETURN_ROOT}/stage23a_pose_occ_audit2_failure_return_${PIPELINE_TAG}"
...
package_failure() {
  ...
  echo "STAGE23A_POSE_OCC_AUDIT_STATUS=failed"
...
FAILED_STAGE=pose_occ_shadow_evaluation
...
STAGE23A_EVAL_PORT=${STAGE23A_EVAL_PORT:-2571} \
NPROC_PER_NODE=2 MASTER_PORT=${STAGE23A_MASTER_PORT:-2572} \
...
Stage23A replays one successful stairs route and one flat candidate route. It
updates the deployable GPS+compass SparseOcc and a write-only GT-height audit
copy from identical RGB-D. Habitat pose never enters navigation, candidates,
triage, prompts, or actions. The output includes separate RGB surface,
occupied/free clouds, trajectory, BEV, XZ/YZ views, and pose-height audits.
EOF
find "${WORK_DIR}" -type f | sort > "${WORK_DIR}/RETURN_MANIFEST.txt"
mv "${WORK_DIR}" "${SUCCESS_DEST}"
PIPELINE_COMPLETE=1

echo "STAGE23A_POSE_OCC_AUDIT_STATUS=complete"
