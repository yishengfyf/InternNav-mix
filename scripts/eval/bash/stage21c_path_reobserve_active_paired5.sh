#!/usr/bin/env bash
# Same explicit-seed 5 episodes: Frozen control -> bounded path reobserve active.
...
MANIFEST=${STAGE21C_PATH_ACTIVE_MANIFEST:-scripts/eval/manifests/stage21c_strict_active_5_episode_ids.json}
...
RUNNING_NAME="stage21c_path_reobserve_active_paired5_running_${PIPELINE_TAG}"
SUCCESS_NAME="stage21c_path_reobserve_active_paired5_return_${PIPELINE_TAG}"
FAILURE_NAME="stage21c_path_reobserve_active_paired5_failure_return_${PIPELINE_TAG}"
WORK_DIR="${RETURN_ROOT}/${RUNNING_NAME}"
SUCCESS_DEST="${RETURN_ROOT}/${SUCCESS_NAME}"
FAILURE_DEST="${RETURN_ROOT}/${FAILURE_NAME}"
CONTROL_NAME="compare_vlmap_stage21c_path_reobserve_control_5ep_${PIPELINE_TAG}"
ACTIVE_NAME="compare_vlmap_stage21c_path_reobserve_active_5ep_${PIPELINE_TAG}"
...
Paired active experiment on the identical five explicit-seed strict episodes.
Control: Frozen S2/NextDiT, all recovery paths shadow-only.
Treatment: one strict-only known-free path intervention per episode.
If no path-consistent pixel is visible, at most four turn actions reorient to the
path lookahead; the next observation is checked again. If still invalid, hand
control back to Frozen S2. Occupied/unknown and non-strict events always hold.
EOF
}
...
package_failure() {
  ...
  if [[ -d "${WORK_DIR}" ]]; then
    ...
    echo "STAGE21C_PATH_REOBSERVE_STATUS=failed"
...
STAGE21_EVAL_PORT=${STAGE21C_PATH_CONTROL_EVAL_PORT:-2501} \
NPROC_PER_NODE=4 MASTER_PORT=${STAGE21C_PATH_CONTROL_MASTER_PORT:-2502} \
...
STAGE21_EPISODE_SEED_REPLAY_MANIFEST="${MANIFEST}" \
STAGE21C_SCORER_CHECKPOINT="${CHECKPOINT}" STAGE21C_SCORER_DEVICE=cpu \
STAGE21C_PATH_ACTIVE_EVAL_PORT=${STAGE21C_PATH_ACTIVE_EVAL_PORT:-2503} \
NPROC_PER_NODE=4 MASTER_PORT=${STAGE21C_PATH_ACTIVE_MASTER_PORT:-2504} \
bash scripts/eval/bash/stage21_torchrun_eval.sh \
...
echo "STAGE21C_PATH_REOBSERVE_STATUS=complete"
