...
MANIFEST=${STAGE24A_REPLAY_MANIFEST:-scripts/eval/manifests/stage23a_sensor_smoke1_episode_seed_replay.json}
CONFIG=${STAGE24A_REPLAY_CONFIG:-scripts/eval/configs/habitat_dual_system_vlmap_stage24a_replay_ledger_cfg.py}
...
NPROC=${STAGE24A_NPROC_PER_NODE:-4}
RUN_NAME="compare_vlmap_stage24a_replay_ledger_${PIPELINE_TAG}"
RUN_ROOT="logs/habitat/${RUN_NAME}"
WORK_DIR="${RETURN_ROOT}/stage24a_replay_ledger_smoke1_running_${PIPELINE_TAG}"
SUCCESS_DEST="${RETURN_ROOT}/stage24a_replay_ledger_smoke1_return_${PIPELINE_TAG}"
FAILURE_DEST="${RETURN_ROOT}/stage24a_replay_ledger_smoke1_failure_return_${PIPELINE_TAG}"
...
package_failure() {
  ...
  echo "STAGE24A_REPLAY_STATUS=failed"
...
FAILED_STAGE=replay_ledger_shadow_evaluation
...
STAGE23A_EVAL_PORT=${STAGE24A_EVAL_PORT:-2675} \
NPROC_PER_NODE="${NPROC}" MASTER_PORT=${STAGE24A_MASTER_PORT:-2676} \
bash scripts/eval/bash/stage21_torchrun_eval.sh --config "${CONFIG}"

FAILED_STAGE=replay_ledger_integrity_audit
python3 scripts/eval/analyze_stage24a_replay_ledger.py \
  --run-root "${RUN_ROOT}" \
  --output "${RUN_ROOT}/stage24a_replay_ledger_integrity.json"
...
Stage24A stores audit-only RGB-D, pose, S2 query/output, action and compact
SparseOcc/semantic state. It must not change S2 outputs, actions, safety gates,
or triage. Internal LOOKDOWN sensor steps are represented by one logical action.
EOF
find "${WORK_DIR}" -type f | sort > "${WORK_DIR}/RETURN_MANIFEST.txt"
mv "${WORK_DIR}" "${SUCCESS_DEST}"
PIPELINE_COMPLETE=1
echo "STAGE24A_REPLAY_STATUS=complete"
