#!/usr/bin/env bash
# Exact-seed paired replay for the only treated Stage21c-r2 episode.
# This is a stability check, not a generalization experiment.
...
MANIFEST=${STAGE21C_PATH_ACTIVE_MANIFEST:-scripts/eval/manifests/stage21c_path_reobserve_9450_replay1.json}
...
RUNNING_NAME="stage21c_path_reobserve_active_9450_replay1_running_${PIPELINE_TAG}"
SUCCESS_NAME="stage21c_path_reobserve_active_9450_replay1_return_${PIPELINE_TAG}"
FAILURE_NAME="stage21c_path_reobserve_active_9450_replay1_failure_return_${PIPELINE_TAG}"
WORK_DIR="${RETURN_ROOT}/${RUNNING_NAME}"
SUCCESS_DEST="${RETURN_ROOT}/${SUCCESS_NAME}"
FAILURE_DEST="${RETURN_ROOT}/${FAILURE_NAME}"
CONTROL_NAME="compare_vlmap_stage21c_path_reobserve_control_9450_replay1_${PIPELINE_TAG}"
ACTIVE_NAME="compare_vlmap_stage21c_path_reobserve_active_9450_replay1_${PIPELINE_TAG}"
...
Exact-seed paired replay of XcA2TqTSSAj/9450, the only treated episode in
stage21c_path_reobserve_active_paired5. Control and active use the same
episode_eval_seed=200073. Treatment remains strict-only, one bounded
known-free-path reorient/reobserve intervention, with no blind forward and no
Stage21d context. This replay estimates treatment stability; it is not a
...
package_failure() {
  ...
  if [[ -d "${WORK_DIR}" ]]; then
    ...
    echo "STAGE21C_PATH_REOBSERVE_9450_STATUS=failed"
...
STAGE21_EVAL_PORT=${STAGE21C_PATH_CONTROL_EVAL_PORT:-2511} \
NPROC_PER_NODE=4 MASTER_PORT=${STAGE21C_PATH_CONTROL_MASTER_PORT:-2512} \
...
STAGE21_EPISODE_SEED_REPLAY_MANIFEST="${MANIFEST}" \
STAGE21C_SCORER_CHECKPOINT="${CHECKPOINT}" STAGE21C_SCORER_DEVICE=cpu \
STAGE21C_PATH_ACTIVE_EVAL_PORT=${STAGE21C_PATH_ACTIVE_EVAL_PORT:-2513} \
NPROC_PER_NODE=4 MASTER_PORT=${STAGE21C_PATH_ACTIVE_MASTER_PORT:-2514} \
bash scripts/eval/bash/stage21_torchrun_eval.sh \
...
echo "STAGE21C_PATH_REOBSERVE_9450_STATUS=complete"
