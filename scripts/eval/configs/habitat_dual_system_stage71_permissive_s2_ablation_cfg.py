"""Stage71: explicit counterfactual S2-guided recovery ablation.

This configuration is not a safety release.  During stage65 recovery only, it
lets the frozen S2 pixel drive one NextDiT primitive without SparseOcc/VLMap
endpoint or trajectory veto, so Habitat collision/progress can be measured as
an ablation.  Normal DualVLN execution remains unchanged.
"""
...
run_root = os.environ.get("STAGE71_RUN_ROOT", "/data/usr_data/yifeifeng/internnav/stage_results/runs").rstrip("/")
run_name = os.environ.get("STAGE21_RUN_NAME", "stage71_permissive_s2_ablation")
...
eval_cfg.eval_settings["port"] = os.environ.get("STAGE71_EVAL_PORT", "3710")
