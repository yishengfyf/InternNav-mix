...
def _load_stage22c_cfg():
    path = Path(__file__).with_name(
        "habitat_dual_system_vlmap_stage22c_fixed_route_pitch_occ_shadow_cfg.py"
    )
    spec = importlib.util.spec_from_file_location("_stage22c_cfg", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.eval_cfg


eval_cfg = copy.deepcopy(_load_stage22c_cfg())
...
eval_cfg.eval_settings["port"] = os.environ.get(
    "STAGE22_FIXED_ROUTE_EVIDENCE_EVAL_PORT", "2567"
)
