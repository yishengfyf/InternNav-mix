...
最终标注入口采用独立静态页面，逐项填写后自动暂存于浏览器，支持导出 JSONL。生成器与核心 dataset 对齐：当前 RGB 对应 `raw_actions[t+1]`；相对目标跨度至少为 3 且像素坐标有效时提供 `pixel_goal`；跨度 0--2 以及目标不可用的直行样本跳过。RGB、pose、goal 统一使用 `60cm_15deg`。场景固定为已核验的 `17DRP5sb8fy`，本批次不是跨场景验证。旧批次错误的动作、相机设置和位姿解释不能用于正式标签。
